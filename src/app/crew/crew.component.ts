...
export class CrewComponent implements OnInit {
  ...
  addCrewMember(candObj: object) {
    if (this.crew.includes(candObj)) {
      this.crew.splice(this.crew["name"], 1);
    } else if (!this.crew.includes(candObj) && this.crew.length < 3) {
      this.crew.push(candObj);
    }
  }
}
